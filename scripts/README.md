...
Use `--start` and `--end` to supply explicit ISO timestamps (with fractional seconds), or let the script record both as "now".
...
If jsonschema isn't installed, validation will be skipped but the script will still attempt a safe append.
