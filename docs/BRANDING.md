# Branding Guidelines for Kimberly
...
- **Primary Icon**: Brain with agents and memory waves (see `docs/branding/icon.svg`).
- **Secondary Icons** (to be developed):
  - Memory tier icon (stacked bars).
  - Agent delegation icon (dots in a network).
  - Chat interface icon (speech bubble with AI elements).
  - Security icon (shield with encryption motifs).
- Usage: Icons should be scalable, with a minimum size of 16px for digital use.

## Usage Guidelines

### Do's

- Use the full color palette for digital interfaces.
- Maintain clear space around logos (1x logo height).
- Apply consistent typography across all materials.
- Ensure accessibility (WCAG 2.1 AA compliance).

### Don'ts

- Alter logo colors or proportions.
- Use fonts outside the specified set.
- Apply gradients or effects not in the palette.
...
- **Examples**:
  - Chat responses: "I'm here to help – what can I assist with today?"
  - Error messages: "Oops, something went wrong. Let's try that again."

## Usage Guidelines

- **Consistency**: Use the palette across all platforms and materials.
- **Accessibility**: Ensure high contrast ratios for text on backgrounds.
- **Scalability**: Designs should work from mobile to desktop.
...
- **Full Logo**: Use for marketing, headers, and official communications.
- **Icon Only**: For app icons, favicons, and small UI elements.
- **Monochrome**: Black or white versions for limited-color environments.
- **Clear Space**: Maintain 1x logo height around the logo.
...
- **H1**: Roboto Bold, 32px, Deep Purple.
- **H2**: Roboto Medium, 24px, Dark Grey.
- **Body**: Roboto Regular, 16px, Dark Grey.
- **Links**: Electric Indigo, underlined on hover.
- **Buttons**: White text on Deep Purple background.
...
- [x] Logo files (SVG, PNG in multiple sizes).
- [x] Color swatches (HEX, RGB, CMYK).
- [x] Font files and usage specs.
- [x] Icon library.
- [x] Brand deck template (draft logos accepted).
- [x] Usage do's and don'ts.

## Accepted Draft Logos

- **Logo**: K with memory tiers and agent dots (see `docs/branding/logo.svg`).
- **Icon**: Brain with agent dots and memory waves (see `docs/branding/icon.svg`).
- Status: Draft accepted for further refinement.

## Visual Assets

- **Logo**: See `docs/branding/logo.svg` for the primary logo concept.
- **Icon**: See `docs/branding/icon.svg` for the app icon.
- **Color Swatches**: See `docs/branding/color-swatches.svg` for the full palette.
