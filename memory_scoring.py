...
class MemoryScorer:
    ...
    def _calculate_components(self, memory_item: Dict[str, Any]) -> Dict[str, float]:
        ...
        created_at = datetime.fromisoformat(memory_item['created_at'].replace('Z', '+00:00'))
        last_seen = datetime.fromisoformat(memory_item['last_seen_at'].replace('Z', '+00:00'))
        ...
        frequency = min(days_since_created / 7.0, 1.0)  # Increase with age (up to 1 week)
...
class MeditationEngine:
    ...
    def run_meditation(self, memory_items: List[Dict[str, Any]],
                      quotas: Dict[str, int]) -> Dict[str, Any]:
        """
...
if __name__ == "__main__":
    ...
    # Sample meditation
    items = [sample_item]
    quotas = {'short-term': 512*1024, 'long-term': 2*1024*1024, 'permanent': 10*1024*1024}
