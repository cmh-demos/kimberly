...
def test_chat_endpoint_authorized(mock_redis):
    # Mock login
    login_response = client.post("/token", json={"username": "testuser", "password": "testpass"})
    token = login_response.json()["access_token"]

    headers = {"Authorization": f"Bearer {token}"}
    response = client.post("/chat", json={"message": "Hello", "user_id": "testuser"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_get_memory(mock_redis):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get("/memory?user_id=testuser&memory_type=long-term", headers=headers)
    assert response.status_code == 200
    data = response.json()
