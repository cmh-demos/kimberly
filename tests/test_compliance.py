...
class TestAuditLogger(unittest.TestCase):
  ...
  def test_query_filters_by_action(self):
    ...
    result = self.logger.query(
      user_id="user_123", action=AuditAction.LOGIN
    )
  ...
  def test_query_pagination(self):
    for i in range(15):
      self.logger.log(user_id="user_123", action=AuditAction.DATA_ACCESSED)
...
class TestDataExportService(unittest.TestCase):
  ...
  def test_complete_export_updates_status(self):
    ...
    self.assertEqual(
      status.download_url, "https://example.com/download"
    )
...
class TestRetentionPolicyService(unittest.TestCase):
  ...
  def test_default_policies_have_expected_values(self):
    ...
    self.assertEqual(
      policy_map[DataType.CHAT_HISTORY].retention_days, 365
    )
    self.assertEqual(
  ...
  def test_update_policies_modifies_retention_days(self):
    ...
    self.assertEqual(
      policy_map[DataType.CHAT_HISTORY].retention_days, 180
    )
