...
class AgentQuota:
    ...
    def __init__(self, max_concurrent: int = 3, max_runtime: int = 300,
...
class AgentRunner:
    ...
    async def _run_in_sandbox(self, agent: BaseAgent, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            result = await asyncio.wait_for(
                agent.execute(task),
                timeout=self.quota.max_runtime
            )
